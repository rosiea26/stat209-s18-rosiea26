...
154 observations, 2 per area number for night and non-night crimes. There is one row per every combination of area code and night flag. There are more crimes committed at night than during the day. There are more crimes in general committed in low-income areas than high-income areas. The slopes mean that there is a more drastic difference in crimes committed at night depending on income and a less drastic difference in crimes committed during the day depending on income. 
...
```{r}
crimes_time_ca$burglary_household <- (crimes_time_ca$burglary_sum)/(crimes_time_ca$num_households)*100
```


```{r}
ggplot(crimes_time_ca, aes(median_income, burglary_household)) +
  geom_point(aes(color=night_flag))
```

Thesis: The time of day has a larger effect in low-income neighborhoods with many burglaries happening at night, but the time of day does not largely affect the burglary rate in high-income neighborhoods. 
