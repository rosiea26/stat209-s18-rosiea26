...
Answer: The relationship between the mass of a male bird and the mass of the egg is a positive linear correlation (as one increases, so does the other).
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  annotate("text", x=100, y=3, label="Parrots (red)", size=5) +
  scale_x_log10() +
  scale_y_log10()
```
...
Answer: Yes, this line indicates a positive linear correlation with one variable (egg size) increasing as the other one (size of male bird) does.
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  scale_y_log10() +
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5) +
  annotate("point", x = 218.7, y = 87.00, color="red") +
  annotate("text", x = 218.7, y = 150.00, label = "Red-tailed tropicbird")
```
...
```{r}
mating <- filter(birds, mating_system %in% c(2, 3))
```

```{r}
ggplot(birds, aes(male_mass, egg_mass)) +
  geom_point(color="blue") +
  geom_point(color="pink", data = mating) +
  scale_x_log10() +
  scale_y_log10() +
  annotate("text", x=300, y=2, label = "Birds that are non-monogamous (blue)") +
  annotate("text", x=100, y=200, label = "Birds that are monogamous or mostly-monogamous (pink)") +
  ggtitle("Monogamy Among Birds, by Egg Size and Size of Male") +
  xlab("Male Mass") +
  ylab("Egg Mass")
```

This graph displays the mating systems of birds, either all/mostly monogomous (pink) or non-monogamous (blue). The most birds that are non-monogomaous tend to have middle- to low-mass in the adult males, though there are some large-mass males that are non-monogamous. The species with the lowest masses in adult males, Anna's Hummingbird (4.5) and the Rifleman and Western Gerygone (5.6), are both non-monogamous. The mass of the egg does not appear to be affected by if the bird is monogamous or not, though the eggs with the largest masses are produced by monogomous or mostly-monogamous birds.
