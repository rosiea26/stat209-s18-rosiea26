...
**Answer**: 3rd = 369.00, 97th = 4107.46
...
**Answer**: There is a very slight positive difference between the fast score and the normal and slow speeds. Both are significant. 
...
**Answer**: Some people may not give much thought to their answers, finishing in a matter of minutes. Some other people may give too much thought to their answers or forget about the quiz and come back to it later, not completely finishing and submitting the quiz until an hour from the time they started it.
...
```{r}
model <- lm_basic(emotional_stability ~ 1 + reasoning + sensitivity, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: One would expect that a person with high emotional stability would have a high reasoning score. This matches the positive slope for reasoning. One would think that a person with high emotional stability would have low sensitivity, so the slope would be inverse but this is not the case.

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(emotional_stability ~ 1 + reasoning + sensitivity + speed + gender + fct_lump(country, n=5), data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: As someone's score for emotional stability increases, so do their scores for reasoning and sensitivity. There is little difference between speeds in taking the test, and gender has little effect too. For the most part, the difference in country is statistically insignificant.
